...
public class Main1 {
    ...
    public void test() {
        ...
        System.out.println(method1(maxWeights, weights.length, weights, values));
    }
        ...
        for (int i = 1; i < table.length; i++) {
            for (int j = 1; j < table[0].length; j++) {
                ...
                if (weights[i - 1] > j) {
                    ...
                    //values[i - 1]：当前物品的价值
                    //table[i][j - weights[i - 1]]：放下当前物品之后，剩余空间能放下的最大价值
                    //table[i（无限背包）][j - weights[i - 1]]
                    //table[i - 1（01背包）][j - weights[i - 1]]
